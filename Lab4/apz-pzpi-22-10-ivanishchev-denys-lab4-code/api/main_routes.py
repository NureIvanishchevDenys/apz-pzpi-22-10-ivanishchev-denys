...
def hash_password(password: str) -> str:
    ...
    return hashed.decode('utf-8')  # сохраняем как строку

def verify_password(stored_hash: str, password: str) -> bool:
    stored_hash_fixed = stored_hash.replace('$2y$', '$2b$')  # если вдруг есть старые хеши с $2y$
    return bcrypt.checkpw(password.encode('utf-8'), stored_hash_fixed.encode('utf-8'))
...
def register():
    ...
    user = User(name=name, email=email, password_hash=password_hash, role='analyst', alert_id=None)
    db.session.add(user)
    db.session.commit()
...
def create_user():
    data = request.json
    # Если приходит сырой пароль — его нужно захешировать
